...
public class TestTicket361 extends QuadsTestCase {
    ...
    private void query(final BigdataSailConnection conn) throws SailException, QueryEvaluationException {
        final ProjectionElemList elemList = new ProjectionElemList(new ProjectionElem("z"));
        final TupleExpr query = new Projection(new StatementPattern(new Var("s"), new Var("p"), new Var("o")), elemList);
        final QueryBindingSet bindings = mb("o", "l1", "o", "l2", "o", "l3");
        final CloseableIteration<? extends BindingSet, QueryEvaluationException> results;
        results = conn.evaluate(query, null, new QueryBindingSet(), new Iter(bindings), false, null);
        results.close();
    }
    
    /**
     * Makes a binding set by taking each pair of values and using the first value as name and the second as value.
     * Creates an URI for a value with a ':' in it, or a Literal for a value without a ':'.
     */
    private QueryBindingSet mb(String... nameValuePairs) {
        final QueryBindingSet bs = new QueryBindingSet();
        for (int i = 0; i < nameValuePairs.length; i += 2)
            bs.addBinding(nameValuePairs[i], nameValuePairs[i + 1].indexOf(':') > 0 ? new URIImpl(nameValuePairs[i + 1]) : new LiteralImpl(nameValuePairs[i + 1]));
        return bs;
    }
     ...
     */
    private static class Iter implements CloseableIteration<BindingSet, QueryEvaluationException> {
        ...
        public void close() throws QueryEvaluationException {
            System.out.println("Source iteration closed");
        }
    }
}
