...
//        /**
//         * A directed link to another generic object in the same store.
//         *
//         * @see LinkValue
//         */
//
//        final public static short LINK = 1;
//
//        /**
//         * A BLOB (metadata that supports access to very large binary
//         * objects using a stream oriented API).
//         */
//
//        final public static short BLOB = 2;
        
        /**
         * The Java primitive type <em>boolean</em>.
         *
         * Note: Support for primitives recognizes the corresponding
         * objects, e.g., {@link Boolean}, and special cases their
         * serialization.  This significantly reduces their the storage
         * requirements.
         */
...
//            DOUBLE_ARRAY,
        ...
        public Object nextObject( Random rnd, boolean allowNull )
        {
            ...
            case BYTE: {
                obj = new Byte( (byte) ( rnd.nextInt( 255 ) - 127 ) );
                break;
            }
            case CHAR: {
                obj = new Character( alphabet.charAt( rnd.nextInt( alphabet.length() ) ) );
                break;
            }
            case SHORT: {
                obj = new Short( (short) rnd.nextInt() );
                break;
            }
            case INT: {
                obj = new Integer( rnd.nextInt() );
                break;
            }
            case LONG: {
                obj = new Long( rnd.nextLong() );
                break;
            }
            case FLOAT: {
                obj = new Float( rnd.nextFloat() );
                break;
            }
            case DOUBLE: {
                obj = new Double( rnd.nextDouble() );
                break;
            }
            case OBJECT: {
...
//                DOUBLE_ARRAY
    ...
    private RandomType _randomType = new RandomType();
...
//    MyProperties m_properties;
    ...
    /***********************************************************
     * Deconstructed from class files, source unavailable.
     **********************************************************/
    ...
    public static void assertSameArray(String message, Object[] expected,
        ...
        assertEquals(message + " : length is wrong.",
        ...
        for (int i = 0; i < expected.length; i++) {
            ...
            assertTrue(message + " : expected[" + i + "]=" + expected[i] +
                       ", but actual[" + i + "]=" + actual[i],
                       expected[i].equals(actual[i]));
        }
    }

    protected static Throwable getInnerCause(Throwable t, Class cls) {
        if (t == null) {
    ...
    private static int getUlps(float A, float B) {

        int aInt = Float.floatToIntBits(A);
        if (aInt < 0) {
            aInt = -2147483648 - aInt;
        }

        int bInt = Float.floatToIntBits(B);
        if (bInt < 0) {
            bInt = -2147483648 - bInt;
        }
    ...
    private static long getUlps(double A, double B) {

        long aLong = Double.doubleToLongBits(A);
        if (aLong < 0L) {
            aLong = -9223372036854775808L - aLong;
        }

        long bLong = Double.doubleToLongBits(B);
        if (bLong < 0L) {
            bLong = -9223372036854775808L - bLong;
        }
    ...
    protected void assertZeroUlps(float f1, float f2) {
        ...
        if (ulps != 0) {
            fail("Expecting zero ulps, but found: " + ulps +
                 "; f1=" + f1 + ", f2=" + f2);
        }
    }
