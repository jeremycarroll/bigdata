...
/**
 * A shard locator service for a named index.
 * <p>
 * The shard locator service maintains locator information for the services
 * containing the sharded data; which are responsible for each partition
...
public interface ShardLocator {
     ...
     *
     * @param newLocators The locator information for the new index
     *                    partitions that were created by the split of
     *                    the old index partition.
     * 
     * @throws IOException
     * @throws InterruptedException
     * @throws ExecutionException
     ...
     * @param name        The name of the scale-out index.
     *
     * @param oldLocators The partition locators for the index partitions
     *                    that were joined.
     *
     * @param newLocator  The locator for the new index partition created
     *                    by that join.
     * 
     * @throws IOException
     * @throws InterruptedException
     ...
     * 
     * @param metadata         The metadata template describing the scale-out
     *                         index, including the name to be assigned to
     *                         that index.
     *
     * @param separatorKeys    The array of separator keys. Each separator
     *                         key is interpreted as an 
     *                         <em>unsigned byte[]</em>. The first entry
     *                         MUST be an empty byte[]. The entries MUST be
     *                         in sorted order.
     *
     * @param dataServiceUUIDs The array of data services onto which each
     *                         partition defined by a separator key will be
     *                         mapped (optional). When given, the #of entries
     *                         in this array MUST agree with the #of entries
     *                         in the <i>separatorKeys</i> array and all
     *                         entries must be non-<code>null</code>. When
     *                         not given, the index partitions will be
     *                         auto-assigned to the discovered data services.
     * 
     ...
     * 
     * @param name The name of the scale-out index to drop.
     */
    void dropScaleOutIndex(String name)
             throws IOException, InterruptedException, ExecutionException;

    /**
     * Returns the partition with the given separator <code>key</code> or
     * <code>null</code> (exact match on the separator key).
     * 
     * @param name      The name of the scale-out index to return.
     *
     * @param timestamp
     *
     * @param key       The separator key (the first key that would go into
     *                  the desired partition).
     * 
     * @return The partition with the given separator <code>key</code> or
     *         <code>null</code>.
     */
     ...
     * @param name      The name of the scale-out index to return.
     * @param timestamp
     * @param key       A key of interest for the desired scale-out index.
     * 
     * @return The partition spanning the given <code>key</code> or
