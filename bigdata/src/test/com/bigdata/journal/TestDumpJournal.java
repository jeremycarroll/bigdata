...
public class TestDumpJournal extends ProxyTestCase<Journal> {
    ...
    public void test_dumpJournal_concurrent_updates() throws Exception {
        ...
        try {
            ...
            final List<FutureTask<Void>> tasks = new LinkedList<FutureTask<Void>>();

            // Setup executor that limits parallelism.
            final LatchedExecutor executor = new LatchedExecutor(
                    src.getExecutorService(), 2/* nparallel */);

            try {

                // Tasks to run.
                tasks.add(new FutureTaskMon<Void>(new DumpTask(src)));
                tasks.add(new FutureTaskMon<Void>(new UpdateTask(src)));
                ...
                log.fatal("Blocking for futures");
