...
logger.log(Level.DEBUG, "YYYYY TRANSACTION SERVICE ServiceImpl: constructor");
if(args == null) {
    logger.debug("YYYYY TRANSACTION SERVICE ServiceImpl: args = NULL ****");
} else {
    logger.debug("YYYYY TRANSACTION SERVICE ServiceImpl: args = "+java.util.Arrays.asList(args));
    for(int i=0; i<args.length; i++) {
        logger.debug("YYYYY TRANSACTION SERVICE ServiceImpl: arg["+i+"] = "+args[i]);
    }
}
...
logger.debug("\n\n!!!! THROWABLE", e);
    ...
    {
	readyState.check();
        embeddedTransactionService.declareResources(tx,shardService,resource);
    }
    ...
    {
	readyState.check();
        return embeddedTransactionService.prepared(tx, shardService);
    }
    ...
    {
	readyState.check();
        return embeddedTransactionService.committed(tx, shardService);
    }

    public long nextTimestamp() throws RemoteException {
	readyState.check();
        return embeddedTransactionService.nextTimestamp();
    }

    public void shutdown() throws RemoteException {
logger.warn("YYYYY TRANSACTION SERVICE ServiceImpl: SHUTDOWN CALLED");
	readyState.check();
	readyState.shutdown();
...
logger.warn("YYYYY TRANSACTION SERVICE ServiceImpl: SHUTDOWN_NOW CALLED");
	readyState.check();
	readyState.shutdown();
...
logger.warn("YYYYY TRANSACTION SERVICE ServiceImpl: KILL CALLED");
	readyState.check();
	readyState.shutdown();
        killDo(status);
    }
...
logger.warn("YYYYY TRANSACTION SERVICE ServiceImpl: DESTROY CALLED");
	readyState.check();
	readyState.shutdown();
    ...
    private void init(String[] args) throws Exception {
        ...
        //properties object for the EmbeddedTransactionService
        ...
        embeddedTransactionService = 
            new EmbeddedTransactionService
...
//BTM        waitThread.start();
    ...
    private class ShutdownThread extends Thread {
        ...
        public ShutdownThread(ShutdownType type) {
            super("transaction service shutdown thread");
            setDaemon(false);
            this.type = type;
        }
        ...
        public void run() {
            ...
            switch (type) {
                case GRACEFUL:
                    embeddedTransactionService.shutdown();
                case IMMEDIATE:
                    embeddedTransactionService.shutdownNow();
                case DESTROY_STATE:
                    embeddedTransactionService.destroy();
                    break;
                default:
...
//BTM            } catch (InterruptedException e) {/*exiting, so swallow*/}
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            logger.log(Level.WARN, "failed to start transaction service", t);
        }
    }
}
