...
System.out.println("\nSSSSS SHARD SERVICE ServiceImpl: constructor");
if(args == null) {
    System.out.println("SSSSS SHARD SERVICE ServiceImpl: args = NULL ****");
} else {
    System.out.println("SSSSS SHARD SERVICE ServiceImpl: args.length = "+args.length);
    for(int i=0; i<args.length; i++) {
        System.out.println("SSSSS SHARD SERVICE ServiceImpl: arg["+i+"] = "+args[i]);
    }
}
...
//BTM - had to add this method because of the test TestBigdataClient; which
//BTM - instantiates this class directly (instead of discovering the proxy)
//BTM - and then makes a call to getServiceUUID on that backend object.
//BTM - If/when the test infrastructure and bigdata codebase is changed to
//BTM - no longer need this method, it should be removed.
...
//BTM - PRE_FRED_3481    public Future<? extends Object> submit(Callable<? extends Object> proc)
	...
	readyState.check();
        ...
        return Util.wrapFuture( exporter, embeddedShardService.submit(task) );
    }
	...
	readyState.check();
        ...
        return Util.wrapFuture( exporter, 
                                embeddedShardService.submit(tx, name, proc) );
    }
...
logger.warn("SSSSS SHARD SERVICE ServiceImpl: SHUTDOWN CALLED");
	readyState.check();
	readyState.shutdown();
...
logger.warn("SSSSS SHARD SERVICE ServiceImpl: SHUTDOWN_NOW CALLED");
	readyState.check();
	readyState.shutdown();
...
logger.warn("SSSSS SHARD SERVICE ServiceImpl: KILL CALLED");
	readyState.check();
	readyState.shutdown();
        killDo(status);
    }
...
logger.warn("SSSSS SHARD SERVICE ServiceImpl: DESTROY CALLED");
	readyState.check();
	readyState.shutdown();
    ...
    private void init(String[] args) throws Exception {
        ...
        //add the proxy id for uniqueness to allow for multiple services
        String persistDir = 
        ...
        //properties object for the EmbeddedShardService
        Properties props = new Properties();
        props.setProperty(EmbeddedShardService.Options.DATA_DIR, dataDir);
        ...
        int httpdPort = Config.getIntEntry(config,
                                           COMPONENT_NAME,
                                           "httpdPort",
                                           DEFAULT_HTTPD_PORT, 
                                           LOWER_BOUND_HTTPD_PORT,
                                           UPPER_BOUND_HTTPD_PORT);

        this.sdm = new ServiceDiscoveryManager(ldm, null, config);
        embeddedShardService = 
            new EmbeddedShardService
                    (proxyId, hostname,
                     sdm, //for discovering txn, lbs, mds
                     null,//embedded txn - for embedded fed testing
                     null,//embedded lbs - for embedded fed testing
                     threadPoolSize,
                     indexCacheSize,
                     indexCacheTimeout,
                     metadataIndexCachePolicy,
...
//        waitThread.start();
    ...
    private class ShutdownThread extends Thread {
        ...
        public ShutdownThread(ShutdownType type) {
            super("shard service Shutdown thread");
            setDaemon(false);
            this.type = type;
        }
        ...
        public void run() {
            ...
            switch (type) {
                case GRACEFUL:
                    embeddedShardService.shutdown();
                case IMMEDIATE:
                    embeddedShardService.shutdownNow();
                case DESTROY_STATE:
                    embeddedShardService.destroy();
                    break;
                default:
