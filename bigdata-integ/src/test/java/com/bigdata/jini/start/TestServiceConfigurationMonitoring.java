...
public class TestServiceConfigurationMonitoring extends AbstractFedZooTestCase {
    ...
    public void test_logicalServiceWatcher() throws KeeperException,
        ...
        final String zconfig = fed.getZooConfig().zroot + BigdataZooDefs.ZSLASH + BigdataZooDefs.CONFIG;
         ...
         */
        System.out.println("Creating zpath [serviceImplRemote="+serviceImplRemote+"]: " +serviceConfigurationZPath);
        String zp = null;
        if(serviceImplRemote) {
            zp = zookeeper.create(serviceConfigurationZPath, SerializerUtil
        ...
        System.out.println("Created zpath: " + zp);
        ...
        Assert.assertEquals(1, zookeeper.getChildren(serviceConfigurationZPath, false).size());
        ...
        if (f.isDone()) {
            f.get();
            Assert.fail("not expecting task to end by itself.");
        } else
        ...
        Assert.assertEquals(numBefore + 1, listener.running.size());      
    }
}
